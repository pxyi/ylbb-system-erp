...
export class SatisfactionComponent implements OnInit {
  ...
  saveRatio(data) {
    data.loading = true;
    this.http.post('http://qnewbss.beibeiyue.cn/schedule/schedule/yeqs/bonusSatisfaction/editBonusSatisfaction', { paramJson: JSON.stringify({ id: data.id, ratio: data.ratio }) }).then(res => {
      data.loading = false;
      data.edit = false;
