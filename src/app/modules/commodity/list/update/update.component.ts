...
export class UpdateComponent implements OnInit {
  ...
  ngOnInit() {
    this.formGroup = this.fb.group({
      ...
      cardTimesFlag: [0, [Validators.required]],                 // 是否计次消费 0:’是’,1:’否’
      chargeType: [1, [Validators.required]],                  //	商品类型
      integral: [0, [Validators.required]],                  // 是否积分 计积分 0:’不记积分’,1:’记积分’
      defaultTag: [0, [Validators.required]],                  // 是否设置为默认 是否默认 0:’否’,1:’是’
      introduction: [],                                        //	商品说明
    });
    this.formTypeChange(this.formGroup.get('chargeType').value);
    this.formGroup.get('chargeType').valueChanges.subscribe(val => this.formTypeChange(val) );
  ...
  formTypeChange(val) {
    if (val == 0) {
      this.formGroup.patchValue({ cardTimesFlag: 1 });
      this.formGroup.addControl('type', new FormControl(this.commodityInfo.id ? this.commodityInfo.type : null, [Validators.required]));
      ...
      this.formGroup.patchValue({ cardTimesFlag: 0 });
