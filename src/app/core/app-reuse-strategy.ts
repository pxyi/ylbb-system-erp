...
export class AppReuseStrategy implements RouteReuseStrategy {
  ...
  shouldDetach(route: ActivatedRouteSnapshot): boolean {
    ...
    if (url === 'login') { AppReuseStrategy.routeCache.clear(); }
    const noReuse = ['home', 'login', 'home/index', 'home/password', 'home/analysis', 'home/analysis', 'home/marketing/list']
    return (!data || !data.noReuse) && noReuse.indexOf(url) === -1;
  }
  ...
  retrieve(route: ActivatedRouteSnapshot): DetachedRouteHandle {
    const url = this.getFullRouteUrl(route);
    return AppReuseStrategy.routeCache.has(url) ? AppReuseStrategy.routeCache.get(url).handle : null;
  }
